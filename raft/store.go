...
const (
	BuntLow    BuntLevel = -1
	BuntMedium BuntLevel = 0
	BuntHigh   BuntLevel = 1
)
...
func (b *BuntStore) GetLog(idx uint64, log *Log) error {
	var val string
	var verr error
	err := b.db.View(func(tx *buntdb.Tx) error {
...
func (b *BuntStore) StoreLog(log *Log) error {
	return b.StoreLogs([]*Log{log})
}

// StoreLogs is used to store a set of raft logs
func (b *BuntStore) StoreLogs(logs []*Log) error {
...
func decodeLog(s string, in *Log) error {
	buf := []byte(s)
	if len(buf) < 17 {
	...
	in.Type = LogType(buf[16])
	in.Data = buf[17:]
	return nil
}

// Encode writes an encoded object to a new bytes buffer
func encodeLog(in *Log) ([]byte, error) {
