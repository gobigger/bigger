...
START:
	...
	if err := r.trans.AppendEntries(s.peer, &req, &resp); err != nil {
		...
		// r.logger.Printf("无法追加日志至 [%v]", s.peer)
		//因为连接失败会一直输出LOG，这条就不输出了
		s.failures++
		return
	}
	...
	if resp.Success {
		...
		// r.logger.Printf("追加旧日志 [%d] 至 [%v] 被拒绝", s.nextIndex, s.peer)
	}
...
SEND_SNAP:
	if stop, err := r.sendLatestSnapshot(s); stop {
		...
		r.logger.Printf("无法将快照发送至 [%v]", s.peer)
		return
	}
...
func (r *Raft) sendLatestSnapshot(s *followerReplication) (bool, error) {
	...
	if err != nil {
		...
		r.logger.Printf("无法打开快照 [%v]", snapID)
		return false, err
	}
	...
	if err := r.trans.InstallSnapshot(s.peer, &req, &resp, snapshot); err != nil {
		...
		r.logger.Printf("快照 [%v] 安装失败", snapID)
		s.failures++
		return false, err
	}
	// 待处理
	...
	if resp.Success {
		...
		r.logger.Printf("安装快照至 [%v] 被拒绝", s.peer)
	}
	return false, nil
}
...
func (r *Raft) heartbeat(s *followerReplication, stopCh chan struct{}) {
	...
	for {
		...
		if err := r.trans.AppendEntries(s.peer, &req, &resp); err != nil {
			...
			r.logger.Printf("无法发送心跳至 [%v]", s.peer)
			failures++
			select {
...
func (r *Raft) pipelineReplicate(s *followerReplication) error {
	...
	r.logger.Printf("同步数据到节点 [%v]", s.peer)
	defer r.logger.Printf("对节点 [%v] 的数据同步中断", s.peer)
...
func (r *Raft) handleStaleTerm(s *followerReplication) {
	...
	r.logger.Printf("节点 [%v] 开始新的任期，停止复制", s.peer)
