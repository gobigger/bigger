...
func (r *Raft) String() string {
	return fmt.Sprintf("点节 [%s/%v]", r.localAddr, r.getState())
}
...
func (r *Raft) runFollower() {
	...
	currLeader := r.Leader()
	if currLeader != "" {
		r.logger.Printf("[%v] 现在跟随领导者 [%v]", r.localAddr, r.Leader())
	}
	...
	for {
		...
		case <-heartbeatTimer:
			...
			if len(r.peers) == 0 && !r.conf.EnableSingleNode {
				if !didWarn {
					r.logger.Printf("没有已知的节点，选举失败")
					didWarn = true
				}
			} else {
				if lastLeader != "" {
					r.logger.Printf(`现任领导 [%v] 心跳超时，重新选举`, lastLeader)
				} else {
					r.logger.Printf(`无领导者，开始选举`)
				}
...
func (r *Raft) runCandidate() {
	...
	// r.logger.Printf("[%v] 进入选举状态，共需要 [%d] 票", r.localAddr, votesNeeded)
	...
	for r.getState() == Candidate {
		...
		case vote := <-voteCh:
			...
			if vote.Granted {
				grantedVotes++
				// r.logger.Printf("收到 [%s] 的投票，得票数 [%d]", vote.voter, grantedVotes)
			}

			// Check if we've become the leader
			if grantedVotes >= votesNeeded {
				// r.logger.Printf("选举成功，得票数[%d]", grantedVotes)
				r.setState(Leader)
				r.setLeader(r.localAddr)
				return
			}
...
func (r *Raft) runLeader() {
	r.logger.Printf("[%v] 进入 [领导者] 状态", r.localAddr)
	...
	if r.conf.DisableBootstrapAfterElect && r.conf.EnableSingleNode {
		// r.logger.Printf("Disabling EnableSingleNode (bootstrap)")
		r.logger.Printf("禁用单点模式")
...
func (r *Raft) checkLeaderLease() time.Duration {
	...
	for _, f := range r.leaderState.replState {
		...
		//这是貌似死循环了，contacted一直加， 但是判断在下面的代码
		if diff <= r.conf.LeaderLeaseTimeout {
			...
			//因为有心跳包发送，如果发送失败会log，所以这里就不打日志了，要不然日志太多了
			...
			if diff <= 3*r.conf.LeaderLeaseTimeout {
				...
				// r.logger.Printf("联系 [%v] 已超时 [%v]", peer, diff)
			} else {
	...
	for idx := r.getLastApplied() + 1; idx <= index; idx++ {
		...
		if future != nil && future.log.Index == idx {
			...
			if err := r.logs.GetLog(idx, l); err != nil {
				...
				r.logger.Printf("获取日志失败，%d: %v", idx, err)
				panic(err)
			}
...
func (r *Raft) processLog(l *Log, future *logFuture, precommit bool) (stepDown bool) {
	...
	case LogRemovePeer:
		...
		// r.logger.Printf("节点 [%v] 更新节点设置 [%v/%v]", r.localAddr, l.Type, peers)
		...
		if r.getState() == Leader {
			for _, p := range r.peers {
				if _, ok := r.leaderState.replState[p]; !ok {
					...
					r.logger.Printf("新节点 [%v] 加入，准备复制", p)
		...
		if r.getState() == Leader && !precommit {
			...
			for _, repl := range r.leaderState.replState {
				if !PeerContained(r.peers, repl.peer) {
					...
					r.logger.Printf("删除节点 [%v] 并停止复制", repl.peer)
...
func (r *Raft) processRPC(rpc RPC) {
	...
	default:
		...
		r.logger.Printf("收到未知的命令: %#v", rpc.Command)
...
func (r *Raft) processHeartbeat(rpc RPC) {
	...
	default:
		...
		r.logger.Printf("等待心跳，得到命令：%#v", rpc.Command)
		rpc.Respond(nil, fmt.Errorf("unexpected command"))
	}
}
	...
	if a.PrevLogEntry > 0 {
		...
		if a.PrevLogEntry == lastIdx {
			...
			if err := r.logs.GetLog(a.PrevLogEntry, &prevLog); err != nil {
				...
				// r.logger.Printf("获取上一条日志失败：%d %v (last: %d)", a.PrevLogEntry, err, lastIdx)
		...
		if a.PrevLogTerm != prevLogTerm {
			// r.logger.Printf("[WARN] raft: Previous log term mis-match: ours: %d remote: %d", prevLogTerm, a.PrevLogTerm)
			r.logger.Printf("上一条日志不匹配: 本地: %d 远程: %d", prevLogTerm, a.PrevLogTerm)
			resp.NoRetryBackoff = true
			return
		}
	}
...
func (r *Raft) requestVote(rpc RPC, req *RequestVoteRequest) {
	...
	if leader := r.Leader(); leader != "" && leader != candidate {
		// r.logger.Printf("[WARN] raft: Rejecting vote request from %v since we have a leader: %v", candidate, leader)
		r.logger.Printf("拒绝 [%v] 的投票请求，已有领导者 [%v]", candidate, leader)
		return
	}
	...
	if lastTerm > req.LastLogTerm {
		// r.logger.Printf("[WARN] raft: Rejecting vote request from %v since our last term is greater (%d, %d)", candidate, lastTerm, req.LastLogTerm)
		r.logger.Printf("拒绝 [%v] 的投票请求，因为上一届任期更大(%d, %d)", candidate, lastTerm, req.LastLogTerm)
		return
	}

	if lastTerm == req.LastLogTerm && lastIdx > req.LastLogIndex {
		// r.logger.Printf("[WARN] raft: Rejecting vote request from %v since our last index is greater (%d, %d)", candidate, lastIdx, req.LastLogIndex)
		r.logger.Printf("拒绝 %v 的投票请求，因为本地索引更大(%d, %d)", candidate, lastIdx, req.LastLogIndex)
		return
	}
...
func (r *Raft) installSnapshot(rpc RPC, req *InstallSnapshotRequest) {
	...
	if req.Term < r.getCurrentTerm() {
		...
		r.logger.Printf("忽略旧任期为[%d]和当前任期为[%d]的快照请求", req.Term, r.getCurrentTerm())
		return
	}
	...
	if n != req.Size {
		...
		r.logger.Printf("无法收到完整的快照[%d/%d]", n, req.Size)
	...
	askPeer := func(peer string) {
		r.goFunc(func() {
			...
			if err != nil {
				...
				r.logger.Printf("向 [%v] 请求投票失败", peer)
				resp.Term = req.Term
				resp.Granted = false
			}
			...
			if err == nil {
				...
				if !PeerContained(peerSet, r.localAddr) {
					// r.logger.Printf("[WARN] raft: Remote peer %v does not have local node %v as a peer", peer, r.localAddr)
					r.logger.Printf("远程节点 [%v] 无本地 [%v] 做为对等节点", peer, r.localAddr)
				}
			}
...
func (r *Raft) shouldSnapshot() bool {
	...
	if err != nil {
		...
		r.logger.Printf("无法获取最新日志索引[%v]", err)
		return false
	}
...
func (r *Raft) takeSnapshot() error {
	...
	r.logger.Printf("开始快照直到[%d]", req.index)
	...
	if err != nil {
		...
		return fmt.Errorf("快照创建失败[%v]", err)
	}
	...
	if err := req.snapshot.Persist(sink); err != nil {
		...
		return fmt.Errorf("快照持久化失败[%v]", err)
	}
	...
	if err := sink.Close(); err != nil {
		...
		return fmt.Errorf("快照关闭失败[%v]", err)
	}
	...
	r.logger.Printf("快照完成至[%d]", req.index)
	return nil
}
...
func (r *Raft) compactLogs(snapIdx uint64) error {
	...
	if err != nil {
		...
		return fmt.Errorf("无法获取第一个日志索引[%v]", err)
	}
	...
	r.logger.Printf("压缩日志从[%d]到[%d]", minLog, maxLog)
...
func (r *Raft) restoreSnapshot() error {
	...
	for _, snapshot := range snapshots {
		...
		if err != nil {
			...
			r.logger.Printf("快照打开失败[%v]:[%v]", snapshot.ID, err)
			continue
		}
		defer source.Close()
		...
		if err := r.fsm.Restore(source); err != nil {
			...
			r.logger.Printf("快照恢复失败[%v]:[%v]", snapshot.ID, err)
			continue
		}

		// Log success
		// r.logger.Printf("[INFO] raft: Restored from snapshot %v", snapshot.ID)
		r.logger.Printf("快照[%v]恢复成功", snapshot.ID)
