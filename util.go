...
func parseDuration(s string) (time.Duration, *Error) {
	...
	if s == "" {
		return 0, Bigger.Erring("time: invalid duration " + orig)
	}
	for s != "" {
		var (
			v, f  int64       // integers before, after decimal point
		...
		if i == 0 {
			return 0, Bigger.Erring("time: missing unit in duration " + orig)
		}
		u := s[:i]
		s = s[i:]
		unit, ok := unitMap[u]
		if !ok {
			return 0, Bigger.Erring("time: unknown unit " + u + " in duration " + orig)
		}
		if v > (1<<63-1)/unit {
			// overflow
			return 0, Bigger.Erring("time: invalid duration " + orig)
